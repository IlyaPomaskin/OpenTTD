...
- **Instrumentation/debug KEPT.** `_gles_perf`, PERF logging, GPU timer queries, debug overlays, [CTX]/[LOAD] logs, crash handler all stay (matched to tools/). May move behind a compile-time define later, but do not drop.
