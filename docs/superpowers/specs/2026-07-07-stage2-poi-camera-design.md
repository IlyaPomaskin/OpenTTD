...
**Resolved by Stage 1 (Q2.3):** as of the device-gate fix in `634dd21703`, `ProcessOverlayActions` **already takes `std::lock_guard<std::mutex> lock(this->game_state_mutex)`** (`sdl2_gles_v.cpp:467`) around every scan-triggering call — after a lock-free "nothing pending" fast path (`:459-462`). So **both** entry points into the scanner are already serialized against game-thread map mutation:
- **Draw-thread / JNI-triggered** (jump / navigate / rotate) → funnel through `ProcessOverlayActions`, now under `game_state_mutex`.
...
The mitigation ladder's rung-2 is therefore **already in place** for the paths that exist; residual risk is **low**, and now chiefly about *latency* rather than *correctness*.

**Mitigation ladder (ship the lowest that passes the device gate):**
1. Port the scanner as-is; the existing `ProcessOverlayActions` mutex guard covers it. Verify no crash over extended run + repeated `SWITCH_MAP`/`NEXT_POI`, and that the longer real scan held under `game_state_mutex` does not visibly stall the game thread.
2. If a *new* scan path is introduced that bypasses `ProcessOverlayActions`, or if holding the mutex across the full 648-line scan proves too coarse, guard `ScanMapPOIs` itself (or scope the lock tighter). Kept in **Stage 2** scope as the defined fallback, not punted to Stage 5.
