...
GLuint GLESBackend::LinkProgram(GLuint vert, GLuint frag)
{
	 ...
	 * This allows PaintFBO() to set up vertex attribs once instead of per-batch. */
...
void GLESBackend::QueueDraw(const GLESDrawCommand &cmd)
{
	 ...
	 * May be nullptr if not yet uploaded — corrected during PaintFBO batching. */
...
bool GLESBackend::PaintFBO()
{
	...
	if (this->has_timer_query && this->gpu_query_active) {
		...
		if (available) {
			...
			if (!disjoint) {
				...
				GLES_PERF_COUNT(_gles_perf.gpu_time_us = static_cast<int64_t>(elapsed_ns / 1000));
			}
		}
	}
	...
	if (did_full_render) {
		/* Full sprite render into FBO: build batched vertices, preserving
		 * original Z-order from the viewport (no sorting). Batches only
		 * break on shader type / remap-table changes. */
		...
		for (const GLESDrawCommand &cmd : this->draw_queue) {
			...
			if (gpu_scaled) {
				GLES_PERF_COUNT(_gles_perf.gpu_scaled_hits++);
			} else {
				GLES_PERF_COUNT(_gles_perf.gpu_scaled_fallbacks++);
			}
		...
		for (const BatchRange &b : batches) {
			GLES_PERF_COUNT(_gles_perf.gpu_batches++);
		...
		this->palette_dirty = false;
		GLES_PERF_COUNT(_gles_perf.full_renders++);
	} else if (this->palette_dirty && this->fbo_has_content) {
		/* Palette resolve pass: camera static, only palette changed.
		 * Read palette index from attachment 1, look up updated palette,
		 * write to attachment 0. */
		glBindFramebuffer(GL_FRAMEBUFFER, this->fbo);
		glViewport(0, 0, this->screen_width, this->screen_height);
		...
		this->palette_dirty = false;
		GLES_PERF_COUNT(_gles_perf.gpu_batches++);
		GLES_PERF_COUNT(_gles_perf.resolve_passes++);
	} else {
		/* Nothing changed. */
#ifdef WALLPAPER_PERF
		if (this->has_timer_query) {
