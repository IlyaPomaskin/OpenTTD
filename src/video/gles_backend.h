...
public:
	...
	/** Render sprites into the persistent FBO (+ palette resolve).
	 *  @return true if FBO was updated (full render or resolve),
	 *          false if idle (nothing changed). */
	bool PaintFBO();

	/** Blit the persistent FBO to the screen. */
	void BlitToScreen();
	...
	/** Mark palette as dirty, triggering a resolve pass in PaintFBO(). */
